...
def dbInsert(orderType, orderPrice, takeProfit, stoploss, orderStatus, createdAt):
    # orderType, orderPrice,takeProfit,stoploss,orderStatus,
    # print(orderType, orderPrice, takeProfit, stoploss, orderStatus)
    my_dict = {"orderType": orderType, "orderPrice": str(orderPrice),
               "takeProfit": str(takeProfit), "stoploss": str(stoploss), "orderStatus": orderStatus, "createdAt": (createdAt)}
...
def startAlgorithm():
    ...
    if t1HighFlag:
        if t1Close <= t1Open:
            ...
            dbInsert("Sell", sellOrder, (sellOrder-4),
                     (t1High+2), 'Open', time_arr[0][0])
        # sellOrder(t1Low - 1)
    if t1LowFlag:
        if t1Close >= t1Open:
            ...
            dbInsert("Buy", sellOrder, (sellOrder+4),
                     (t1Low-2), "Open", time_arr[0][0])
...
my_db = my_client["orderManagement"]
