...
class RestClient(object):
    ...
    def request(self, action, data):
        ...
        # if action.startswith("/api/v1/private/"):  v1 is now deprecated
        if action.startswith("/api/v1/public/auth"):
            if self.key is None or self.secret is None:
                raise Exception("Key or secret empty")

            signature = self.generate_signature(action, data)
            response = self.session.post(
                self.url + action, data=data, headers={'x-deribit-sig': signature}, verify=True)
        else:
        ...
        if "error" in json:
            ...
            error = json["error"]
            print("err",error)
            raise Exception("Error (code " + error["code"] + "): " + error["message"])
            return error.data

        if "result" in json:
            return json["result"]
        # elif "message" in json:
        #    return json["message"]     "message" field has been moved to error object
        else:
            return "Ok"

    # generate_signature method appears to be exactly the same for v2
    ...
    def index(self):
        return self.request("/api/v2/public/index", {})

    def stats(self):
        return self.request("/api/v1/public/stats", {})

    def account(self):
        return self.request("/api/v2/private/get_account_summary", {})
